    ...
    if request.method == "POST":
        ...
        if form.is_valid():
            if form.cleaned_data["search"] == "":
                data = (
                    CustomUser.objects.exclude(Q(pk=self_pk) | Q(username="admin"))
                    .annotate(
                        latest_msg=Subquery(
                            Messages.objects.filter(
                                Q(message_from=self_pk, message_to=OuterRef("pk"))
                                | Q(message_from=OuterRef("pk"), message_to=self_pk)
                            )
                            .order_by("-time")
                            .values("message")[:1],
                        ),
                    ...
                    .order_by("blank", "-latest_msg_time", "-date_joined")
                )
                ...
                data = (
                    ...
                    .exclude(Q(pk=self_pk) | Q(username="admin"))
                    .annotate(
                        latest_msg=Subquery(
                            Messages.objects.filter(
                                Q(message_from=self_pk, message_to=OuterRef("pk"))
        ...
        form = SearchForm(request.POST)
        self_pk = int(request.user.pk)  # 自分自身を取得
        data = (
            CustomUser.objects.exclude(Q(pk=self_pk) | Q(username="admin"))
            .annotate(
                latest_msg=Subquery(
...
def talk_room(request,my_pk,pk):
    ...
    data = Messages.objects.filter(
        Q(message_to=pk, message_from=self_pk) | Q(message_to=self_pk, message_from=pk)
    ).order_by(
        "time"
    )  # getはひとつだけ!!
